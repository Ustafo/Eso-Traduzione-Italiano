...
; --- Selezione file traduzione corretta ---
...
;arrayaa := []
;arrayaa := ApplicaFiltroSaltaTraduzione(arrayaa, arrayaa)


; --- Selezione file traduzione consolidata (rimane com’è) ---
...
; ===============================================
; GENERAZIONE FILE LUA CON MARCATORI PERFETTI
; ===============================================
...
for i, row in dataNew {
    if (row[1] = 4330293 || row[1] = 10860933 || row[1] = 162658389 || row[1] = 28666901 || row[1] = 49656629 || row[1] = 121975845 || row[1] = 8290981) {
        ...
        ; === LOGICA MARCATORI VECCHIA PERFETTA ===
        ...
        aggiungii := 1
        if (InStr(enText, "^f") || InStr(enText, "^fd") || InStr(enText, "^m") || InStr(enText, "^md") || InStr(enText, "^ms") || InStr(enText, "^fp") || InStr(enText, "^mp") || InStr(enText, "^np"))
            aggiungii := 0

        chosen := DecideMarker(key, itRaw, frRaw, spRaw)

        itStrip := RegExReplace(itNormalized, "\s*\^[A-Za-z]{1,4}$", "")
        itStrip := Trim(itStrip)
        wordGender := GetGenderFromWord(itStrip)
        ...
        if (newMarker != "" && (newMarker != currentItMarker || aggiungii = 1)) {
            finalMarker := (aggiungii = 1) ? "i" . newMarker : newMarker
            itNormalized := itStrip . " ^" . finalMarker

            if (aggiungii = 1) {
                itNormalized := StrReplace(itNormalized, " ^f", " ^if")
                itNormalized := StrReplace(itNormalized, " ^fd", " ^ifd")
                itNormalized := StrReplace(itNormalized, " ^m", " ^im")
                itNormalized := StrReplace(itNormalized, " ^md", " ^imd")
                itNormalized := StrReplace(itNormalized, " ^ms", " ^ims")
                itNormalized := StrReplace(itNormalized, " ^fp", " ^ifp")
                itNormalized := StrReplace(itNormalized, " ^mp", " ^imp")
                itNormalized := StrReplace(itNormalized, " ^np", " ^inp")
            }
    ...
    if (Mod(i, 20000) = 0)
        Tooltip, Lua: %i% / %totalNew%, 0, 0
}

; Elimina doppioni zone + genera file Lua (identico al tuo originale)
...
outputZoneLua := "TraduzioneItaESO = TraduzioneItaESO or {`n ZoneTranslations = {`n"
if (zoneCount > 0) {
    current := 0
    for itName, enName in uniqueZoneTranslations {
        ...
        outputZoneLua .= "[ """ . itEsc . """ ] = """ . enEsc . """" (current < zoneCount ? "," : "") "`n"
    }
} else
...
if (npcCount > 0) {
    ...
    for key, data in npcTranslations {
        if (data.it && data.en && data.it != data.en) {
            current++
            itEsc := StrReplace(data.it, """", """")
            enEsc := StrReplace(data.en, """", """")
            outputNPCLua .= "[ """ . itEsc . """ ] = """ . enEsc . """" (current < npcCount ? "," : "") "`n"
        }
    }
} else
...
MsgBox, 64, File Lua Generati, Generati %zoneCount% zone e %npcCount% NPC.`nControlla debug_lua.txt se vuoi., 4
...
logFile := A_ScriptDir . "\dacontrollare.txt"
...
if (varconfronto <> "") 
{
    ...
    textItOrig := var5 ; Salva originale per dasistemare.txt
    textIt := textItOrig ; Mantieni originale per output
    ; === CORREZIONE MARCATORI ===
    currentItMarker := ExtractMarkerFromString(textItOrig)
    hasItMarker := (currentItMarker != "")
    if (hasItMarker) 
    {
        ...
        {
            itStrip := RegExReplace(itNormalized, "\s*\^[A-Za-z]{1,4}$", "")
            wordWithoutMarker := Trim(itStrip)
            wordGenderMarker := GetGenderFromWord(wordWithoutMarker)
            if (wordGenderMarker != "") {
                newMarker := wordGenderMarker
            } else {
                newMarker := currentItMarker
            }
        }
        if (newMarker != "" && (newMarker != currentItMarker || aggiungii = 1)) 
        {
            finalMarker := (aggiungii = 1) ? "^i" . newMarker : "^" . newMarker
            itStrip := RegExReplace(itNormalized, "\s*\^[A-Za-z]{1,4}$", "")
            itNormalized := Trim(itStrip) . " " . finalMarker
            if (itNormalized != textItOrig) 
            {
                corrRow := """" . var1 . """,""" . var2 . """,""" . var3 . """,""" . var4 . """,""" . itNormalized . """`n"
                corrContent .= corrRow
            }
            textItOrig := itNormalized
        }
        if (aggiungii = 1) 
        {
            textItOrig := StrReplace(textItOrig, " ^f", " ^if")
            textItOrig := StrReplace(textItOrig, " ^fd", " ^ifd")
            textItOrig := StrReplace(textItOrig, " ^m", " ^im")
            textItOrig := StrReplace(textItOrig, " ^md", " ^imd")
            textItOrig := StrReplace(textItOrig, " ^ms", " ^ims")
            textItOrig := StrReplace(textItOrig, " ^fp", " ^ifp")
            textItOrig := StrReplace(textItOrig, " ^mp", " ^imp")
            textItOrig := StrReplace(textItOrig, " ^np", " ^inp")
        }
    }
    textIt := textItOrig
    ...
    if (itTotalQuotes <= engTotalQuotes && Mod(itTotalQuotes, 2) = 0) 
    {
        ...
        if (textFrOrig != "" && itTotalQuotes <= frTotalQuotes && Mod(itTotalQuotes, 2) = 0) 
        {
        match := true
        matchedQ := itTotalQuotes
    ...
    if (textSpOrig != "" && itTotalQuotes <= spTotalQuotes && Mod(itTotalQuotes, 2) = 0) 
    {
        match := true
        matchedQ := itTotalQuotes
    ...
    if (!match) 
        ...
        if (itTotalQuotes > 0 && !hasSurroundingAny) 
        {
            ...
            if (SubStr(textIt, 1, 1) = """" && SubStr(textIt, -1, 1) = """") 
            {
            ...
            if (res.changed) {
                ...
                if (Mod(candidateQ, 2) = 0 && candidateQ <= maxOtherQ) {
                    if (candidate != textIt) { ; Nuova check: Applica solo se davvero cambiato
                        ...
                        debugMsg .= "CASISTICA 4B APPLICATA: ridotte virgolette esterne a coppia`n"
                        debugMsg .= "IT prima: " textIt " (Q totale=" itTotalQuotes ")`n"
                        debugMsg .= "IT dopo: " candidate " (Q totale=" candidateQ ")`n"
                    } else {
                        debugMsg .= "CASISTICA 4B TENTATA ma NON APPLICATA: Nessun cambiamento effettivo`n"
                    }
                } else {
                    debugMsg .= "CASISTICA 4B TENTATA ma NON APPLICATA: Q=" candidateQ " non valido`n"
                }
            } else {
                debugMsg .= "CASISTICA 4B NON APPLICABILE`n"
            }
        } else {
            debugMsg .= "CASISTICA 4B NON APPLICABILE: Nessuna outer nelle lingue di riferimento`n"
        }
        if (anyFixed) {
            ; Recheck match dopo fix
            match := false
            ...
            if (res.changed) {
                ...
                if (Mod(candQ, 2) = 0 && candQ <= maxOtherQ) {
                    if (cand != srcIt) { ; Nuova: Applica solo se cambiato
                        ...
                        itTotalQuotes := candQ
                        applied := true
                        debugMsg .= "CASISTICA 4C APPLICATA (diretto su IT orig)`n"
                        debugMsg .= "IT orig: " textItOrig " (Q totale originale=" CountAllQuotes(textItOrig, true) ")`n"
                        debugMsg .= "IT dopo: " cand " (Q totale=" candQ ")`n"
                    } else {
                        debugMsg .= "CASISTICA 4C TENTATA ma NON APPLICATA: Nessun cambiamento effettivo`n"
                    }
                }
                if (!applied && candQ = 2 && maxOtherQ >= 4) {
                    ...
                    if (Mod(cand2Q, 2) = 0 && cand2Q <= maxOtherQ) {
                        ...
                        applied := true
                        debugMsg .= "CASISTICA 4C APPLICATA: ridotte a coppia poi aggiunta coppia esterna per Q=4`n"
                        debugMsg .= "IT orig: " textItOrig " (Q totale originale=" CountAllQuotes(textItOrig, true) ")`n"
                ...
                if (!applied) {
                    debugMsg .= "CASISTICA 4C TENTATA ma NON APPLICATA: Q=" candQ " (maxOtherQ=" maxOtherQ ")`n"
                }
            } else {
                debugMsg .= "CASISTICA 4C NON APPLICABILE`n"
            }
        } else {
            debugMsg .= "CASISTICA 4C NON APPLICABILE: Nessuna outer nelle lingue di riferimento`n"
        }
        if (anyFixed) {
            ; Recheck match dopo fix
            match := false
        ...
        if InStr(textIt, va1) || InStr(textIt, va2) || InStr(textIt, va3) || InStr(textIt, va4) {
            ...
            if (normalizedIt != textIt) {
                ...
                if (Mod(newItaQ, 2) = 0 && newItaQ <= maxOtherQ) {
                    ...
                    textIt := normalizedIt
                    itTotalQuotes := newItaQ
                    debugMsg .= "CASISTICA 5 APPLICATA: conversione carattere-per-carattere in virgolette ASCII`n"
                    debugMsg .= "IT dopo: " normalizedIt " (Q totale=" newItaQ ")`n"
                } else {
                    debugMsg .= "CASISTICA 5 TENTATA ma NON APPLICATA: newQ=" newItaQ " (non valido rispetto ai limiti)`n"
                }
            } else {
                debugMsg .= "CASISTICA 5 NON APPLICABILE`n"
            }
        } else {
            debugMsg .= "CASISTICA 5 NON APPLICABILE`n"
        }
...
if (preparatestopergioco) {
    var5 := StrReplace(var5, "battlefield", "Campo di battaglia - Battleground Match")
    var5 := StrReplace(var5, "Summerset", "Isola del Tramonto - Summerset")
    var5 := StrReplace(var5, "Reaper's March", "Marcia del Mietitore - Reaper's March")
    var5 := StrReplace(var5, "Coldharbour", "Porto Gelido - Coldharbour")
    var5 := StrReplace(var5, "Artaeum", "Artaeum")
    var5 := StrReplace(var5, "Greenshade", "OmbraVerde - Greenshade")
    var5 := StrReplace(var5, "Reaper's March", "Passo del Mietitore - Reaper's March")
    var5 := StrReplace(var5, "Stormhaven", "Rifugiodallatempesta - Stormhaven")
    var5 := StrReplace(var5, "Rivenspire", "GugliaSquarciata - Rivenspire")
    var5 := StrReplace(var5, "Glenumbra", "Valle d'Ombra - Glenumbra")
    var5 := StrReplace(var5, "Shadowfen", "PaludeOmbrosa - Shadowfen")
    var5 := StrReplace(var5, "Stonefalls", "Ghiaionefranante - Stonefalls")
    var5 := StrReplace(var5, "Grahtwood", "Bosco di Graht - Grahtwood")
    var5 := StrReplace(var5, "Craglorn", "FalesiaPerduta - Craglorn")
    var5 := StrReplace(var5, "Evermore", "Perpetuamente - Evermore")
    var5 := StrReplace(var5, "The black wood", "Bosco Nero - Black Wood")
    var5 := StrReplace(var5, "black wood", "Bosco Nero - Black Wood")
    var5 := StrReplace(var5, "Eastmarch", "Frontiera Orientale - Eastmarch")
    var5 := StrReplace(var5, "The Rift", "Fenditura Spaccata - The Rift")
    var5 := StrReplace(var5, "Deeshan", "Mostra Cammino - Deeshan")
    var5 := StrReplace(var5, "High Isle", "Isola Alta - High Isle")
    var5 := StrReplace(var5, "West Weald", "Selva di Ponente - West Weald")
    var5 := StrReplace(var5, "Wrothgar", "Terre di Rothgar - Wrothgar")
    var5 := StrReplace(var5, "The Reach", "Le Terre Alte - The Reach")
    var5 := StrReplace(var5, "Murkmire", "Fosca Palude - Murkmire")
    var5 := StrReplace(var5, "Malabal Tor", "Terra Oscura - Malabal Tor")
    var5 := StrReplace(var5, "White-Gold Tower", "Torre d'Oro Bianco - White-Gold Tower")
    var5 := StrReplace(var5, "Selene's Web", "Ragnatele di Selene - Selene's Web")
    var5 := StrReplace(var5, "Blackheart Haven", "Rifugio di Cuorenero - Blackheart Haven")
    var5 := StrReplace(var5, "Direfrost Keep", "Prigione di ghiaccio - Direfrost Keep")
    var5 := StrReplace(var5, "Banished Cells", "Dungeon del Divieto - Banished Cells")
    var5 := StrReplace(var5, "Vaults of Madness", "Volte della Follia - Vaults of Madness")
    var5 := StrReplace(var5, "Moon Hunter Bailey", "Baia del Cacciatore di Luna - Moon Hunter Bailey")
    var5 := StrReplace(var5, "Moon Hunter Keep", "Torrione del Cacciatore di Luna - Moon Hunter Keep")
    var5 := StrReplace(var5, "Elden Hollow", "Cava degli anziani - Elden Hollow")
    var5 := StrReplace(var5, "Blessed Crucible", "Il Crogiolo benedetto - Blessed Crucible")
    var5 := StrReplace(var5, "Fungal Grotto ", "Grotta dei funghi - Fungal Grotto")
    var5 := StrReplace(var5, "The Crypt of hearts", "La Cripta dei cuori - The Crypt of Hearts")
    var5 := StrReplace(var5, "Crypt of Hearts ", "Cripta dei cuori - Crypt of Hearts")
    var5 := StrReplace(var5, "City of Ash", "Città della Cenere - City of Ash")
    var5 := StrReplace(var5, "Darkshade Caverns", "Oscure Caverne - Darkshade Caverns")
    var5 := StrReplace(var5, "Spindleclutch", "Bozzolointrecciato - Spindleclutch")
    var5 := StrReplace(var5, "Tempest Island", "Isola della Tempesta - Tempest Island")
    var5 := StrReplace(var5, "Wayrest Sewers", "Fogne di Wayrest - Wayrest Sewers")
    var5 := StrReplace(var5, "Stone Garden", "Giardino di roccia - Stone Garden")
    var5 := StrReplace(var5, "Balarel Ruins", "Rovine di Balarel - Balarel Ruins")
    var5 := StrReplace(var5, "Silaseli Ruins", "Rovine di Silaseli - Silaseli Ruins")
    var5 := StrReplace(var5, "Root Sunder Ruins", "Rovine di radici spezzate - Root Sunder Ruins")
    var5 := StrReplace(var5, "Icereach", "Gelodistesa - Icereach")
    var5 := StrReplace(var5, "Cradle of Shadows", "Culla di ombre - Cradle of Shadows")
    var5 := StrReplace(var5, "Scalecaller Peak", "Picco del scalatore - Scalecaller Peak")
    var5 := StrReplace(var5, "Bloodroot Forge", "Forgia delle radici sanguinanti - Bloodroot Forge")
}
...
ExtractMarkerFromString(str) {
    ...
    ; Cerca il marcatore alla fine: optional spazi prima di ^, poi ^, poi 1-4 lettere
    ; Poiché RTrim rimuove spazi finali, controlliamo le possibili lunghezze del marcatore
    for i in [1,2,3,4] {
        total := 1 + i  ; ^ + lettere
        if (total > len)
            continue
        
        end := SubStr(str, len - total + 1, total)
        ...
        ; Verifica se marker è composto da 1-4 lettere
        is_letters := true
        marker_len := StrLen(marker)
        Loop % marker_len {
            char := SubStr(marker, A_Index, 1)
            char_lower := Format("{:L}", char)
        ...
        if (is_letters) {
            ...
            ; Se il marcatore inizia con 'i' e ha più di un carattere, rimuovi la 'i'
...
NormalizeMarkerInString(str) {
    ...
    if (str = "")
        return str
    ; Cerca un marcatore alla fine della stringa
    if RegExMatch(str, "(.*)\s*\^([A-Za-z]{1,4})$", m) {
        rest := m1
        marker := m2
        StringLower, marker, marker
        ; Se il marcatore inizia con 'i', lo rimuoviamo per la normalizzazione
        if (SubStr(marker,1,1) = "i" && StrLen(marker) > 1)
            baseMarker := SubStr(marker,2)
        else
            baseMarker := marker
        ; Applica la mappatura
        if (IsObject(markerMap) && markerMap.HasKey(baseMarker))
            newBaseMarker := markerMap[baseMarker]
        else
            newBaseMarker := baseMarker
        ; Ricostruisce la stringa con il marcatore normalizzato
        return rest . " ^" . newBaseMarker
    }
    return str
}

; GetGenderFromWord per AutoHotkey v1.1 - versione semplificata
GetGenderFromWord(word)
{
    ...
    firstWord := ""
    Loop, Parse, word, %A_Space%
    {
        firstWord := A_LoopField
        break
    }

    ; rimuovi punteggiatura e numeri finali dalla prima parola
    mainWord := RegExReplace(firstWord, "[^A-Za-zÀ-ÖØ-öø-ÿ\-]+$")
    if (mainWord = "")
        mainWord := firstWord

    StringLower, mainWordLower, mainWord
    len := StrLen(mainWordLower)
    if (len < 1)
        return ""

    lastChar := SubStr(mainWordLower, len, 1)

    gender := ""
    number := ""

    ; regole base semplici
    if (lastChar = "a")
    {
        gender := "f"
        number := "s"
    ...
    ; suffissi femminili
    femList := ["ione","ura","zia","sia","tà","ice","ite"]
    Loop, % femList.Length()
    {
        s := femList[A_Index]
        if (StrLen(s) <= len)
    ...
    ; suffissi maschili
    mascList := ["ore","aggio","iglio","iere","ame","ume"]
    Loop, % mascList.Length()
    {
        s := mascList[A_Index]
        if (StrLen(s) <= len)
        {
    ...
    ; casi interi femminili (liste ESO)
    esoF := ["torre","cripta","grotta","isola","valle","marcia","pace","chiave","gente","rete","eclissi","concentrazione","protezione","rovine","catacombe","mine","necropoli"]
    Loop, % esoF.Length()
    {
        w := esoF[A_Index]
        if (mainWordLower = w)
        {
            gender := "f"
            if (w = "rovine" || w = "catacombe" || w = "mine" || w = "necropoli")
                number := "p"
            else
                number := "s"
            break
    ...
    ; casi interi maschili (liste ESO)
    esoM := ["ponte","sole","fiume","bosco","dispetto","bastione","santuario","cimitero","tempio","forte","mercato","rifugio","reliquiario","caldera","prigione"]
    Loop, % esoM.Length()
    {
        w := esoM[A_Index]
        if (mainWordLower = w)
    ...
    ; primo carattere della parola principale
    firstChar := SubStr(mainWordLower, 1, 1)
    vowels := "aeiouàèéìòùáíóúäëïöü"
    isVowel := InStr(vowels, firstChar)
    isH := (firstChar = "h")
    ...
    if (len >= 2)
    {
        secondChar := SubStr(mainWordLower, 2, 1)
        if (firstChar = "s" && !InStr(vowels, secondChar))
    ...
    ; mappatura semplice al marcatore
    if (number = "s")
    {
        if (gender = "f")
        {
