...
for i, row in dataNew {
    ...
    if (chosen != "") {
        if (hasItMarker) {
            if (!hasEnMarker) {
                markerLetters := SubStr(currentItMarker, 2)
                newMarker := "^i" . markerLetters
                itStrip := RegExReplace(itNormalized, "\^[A-Za-z]{1,4}\s*$", "")
                itNormalized := RTrim(itStrip) . newMarker
            } else {
                ; Se en ha marker, mantieni o correggi con chosen se diverso
...
if (varconfronto <> "") {
    ...
    if (chosen != "") {
        ...
        newMarker := chosen
        if (!hasEnMarker) {
            newMarkerLetters := SubStr(chosen, 2)
        ...
        if (hasItMarker) {
            ...
            if (wordGender != "") {
                ; Correggi marker in base al genere calcolato
                if (wordGender = "m") {
                    newMarker := "^md" ; o "^m" a seconda delle regole
                } else if (wordGender = "f") {
                    newMarker := "^fd" ; o "^f"
                }
            }
            if (!hasEnMarker) {
            ...
            ; No marker in italiano, aggiungi newMarker (inattivo se !hasEnMarker)
        }
