local addon = {
	name = "TraduzioneItaESO",
	pinType = "TraduzioneItaESOTamrielMapPinType",
	locations = {
		[1] = {
			name = "Tamriel",
			alliance = 999,
...
local function ColorizeEnglish(text)
    local color = TraduzioneItaESO.account.bilingualColor or "FFFFFF"
...
EVENT_MANAGER:RegisterForEvent(addonName, EVENT_ADD_ON_LOADED, function(event, name)
    ...
    d("[TraduzioneItaESO] Addon caricato: versione " .. addon.version)
    InitializeAddon()
...
SLASH_COMMANDS["/testtable"] = function()
    local count = 0
    for _ in pairs(TraduzioneItaESO.translationTable or {}) do count = count + 1 end
