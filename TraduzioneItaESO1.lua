...
HookShrineTooltips()
HookKeepTooltips()
HookFastTravelDialog()
HookHouseTooltips()
HookGroupDungeonTooltips()
end
...
local function HookPoiTooltips()
    ...
    local poiTypes = {
        ...
        MAP_PIN_TYPE_CRAFTING_BLACKSMITH,
        MAP_PIN_TYPE_CRAFTING_CLOTHIER,
        MAP_PIN_TYPE_CRAFTING_ENCHANTER,
        MAP_PIN_TYPE_CRAFTING_ALCHEMY_STATION,
        MAP_PIN_TYPE_CRAFTING_WOODWORKER,
        MAP_PIN_TYPE_CRAFTING_JEWELRY_STATION,
        MAP_PIN_TYPE_QUEST_HUB,
        MAP_PIN_TYPE_QUEST_HUB_COMPLETE,
        MAP_PIN_TYPE_TRACKED_QUEST_OFFER_ZONE_STORY,
        MAP_PIN_TYPE_QUEST_ZONE_STORY_SEEN,
        MAP_PIN_TYPE_QUEST_ZONE_STORY_UNSEEN,
    ...
    for _, pinType in ipairs(poiTypes) do
        if ZO_MapPin.TOOLTIP_CREATORS[pinType] then
            local originalCreator = ZO_MapPin.TOOLTIP_CREATORS[pinType].creator
            ZO_MapPin.TOOLTIP_CREATORS[pinType].creator = function(pin, ...)
                originalCreator(pin, ...)
...
local function HookHouseTooltips()
    if ZO_MapPin.TOOLTIP_CREATORS[MAP_PIN_TYPE_HOUSE] then
...
local function HookGroupDungeonTooltips()
    if ZO_MapPin.TOOLTIP_CREATORS[MAP_PIN_TYPE_GROUP_DUNGEON] then
...
local function HookFastTravelDialog()
...
function addon:Initialize()
    ...
    UpdateMapName()
    addon:InitializeMap()
    -- Fix per multiline in mouseover name e zone name
    ZO_WorldMapMouseoverName:SetMultiLine(true)
    ZO_WorldMapMouseoverName:SetMaxLineCount(2)
    ZO_WorldMapMouseoverName:SetHorizontalAlignment(TEXT_ALIGN_CENTER)
    ZO_WorldMapMouseoverName:SetResizeToFitDescendents(true)
    ZO_WorldMapMouseoverName:SetResizeToFitPadding(0, 10)
    ZO_WorldMapZoneNameLabel:SetMultiLine(true)
    ZO_WorldMapZoneNameLabel:SetMaxLineCount(2)
    ZO_WorldMapZoneNameLabel:SetHorizontalAlignment(TEXT_ALIGN_CENTER)
    ZO_WorldMapZoneNameLabel:SetResizeToFitDescendents(true)
    ZO_WorldMapZoneNameLabel:SetResizeToFitPadding(0, 10)
end
-- Slash commands
