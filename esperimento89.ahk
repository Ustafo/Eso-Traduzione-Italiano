...
preparafile:
; Modificato script AHK con la nuova logica per i marcatori secondo le indicazioni fornite.
; Ho integrato:
; - Definizione dell'insieme A (marcatori di genere, numero, articoli, contrazioni, ecc.) come mappa esplicita.
; - Insieme B per esclusione (tutto ciò che non è in A e segue ^ è considerato B e lasciato com'è).
; - Nuove funzioni: FindMarkerAnywhere (cerca marcatori ovunque, non solo alla fine), ClassifyMarker (classifica A/B, verifica EN e suggerisce azioni), ApplyMarkerSuggestion (applica correzioni come add_i).
; - Sostituito il blocco "=== CORREZIONE MARCATORI - VERSIONE MIGLIORATA ===" in BuildRow con la nuova logica.
; - Mantenuto il resto dello script invariato, inclusa la logica per virgolette (che rimane separata).
; - Aggiunto logging delle correzioni in corrContent come prima.
; - Nella generazione Lua (per zone/NPC) ho applicato una versione semplificata della nuova logica per coerenza, ma conservativa (solo add_i se EN non ha marker e IT ha A).

; -------------------------
; Definizione insieme A (marcatori di genere/numero/articoli/contrazioni)
; -------------------------
markersA := {}
; genere/numero
markersA["f"] := "gender:f" ; femminile singolare
markersA["m"] := "gender:m" ; maschile singolare
markersA["fp"] := "gender:fp" ; femminile plurale
markersA["mp"] := "gender:mp" ; maschile plurale
markersA["np"] := "gender:np" ; neutro/plurale (opzionale)
; genere + determinazione (d = determinato)
markersA["fd"] := "gender:f;det:yes"
markersA["md"] := "gender:m;det:yes"
markersA["fpd"] := "gender:fp;det:yes"
markersA["mpd"] := "gender:mp;det:yes"
; indeterminativi / partitive
markersA["fi"] := "indef:f"
markersA["mi"] := "indef:m"
markersA["pi"] := "indef:pl"
markersA["part"] := "partitive"
; contrazioni / articoli espliciti (mappati come marker A per semplificazione)
markersA["del"] := "contr:del"
markersA["della"] := "contr:della"
markersA["dei"] := "contr:dei"
markersA["degli"] := "contr:degli"
markersA["allo"] := "contr:allo"
markersA["agli"] := "contr:agli"
; segnali per concordanza / elisione / pronomi (opzionali ma utili)
markersA["elide"] := "flag:elide"
markersA["pro"] := "flag:pronoun"
markersA["adjf"] := "flag:adj-f"
markersA["adjm"] := "flag:adj-m"

; -------------------------
; Funzioni per marcatori (nuove)
; -------------------------
FindMarkerAnywhere(str) {
    if (!str)
        return ""
    if RegExMatch(str, "\^i?[A-Za-z]{1,4}", m)
        return m.Value
    return ""
}

ClassifyMarker(itText, enText := "") {
    global markersA
    out := {}
    raw := FindMarkerAnywhere(itText)
    out.found := (raw != "")
    out.raw := raw
    out.enHasMarker := (FindMarkerAnywhere(enText) != "")
    if (!out.found) {
        out.base := ""
        out.hasIPrefix := false
        out.isA := false
        out.isB := false
        out.suggestion := "leave"
        return out
    }
    base := RegExReplace(raw, "^\^i?", "")
    StringLower, baseLower, base
    out.base := baseLower
    out.hasIPrefix := (RegExMatch(raw, "^\^i"))
    out.isA := markersA.HasKey(baseLower)
    out.isB := !out.isA  ; B per esclusione
    if (out.isB) {
        out.suggestion := "leave"
        return out
    }
    if (out.isA) {
        if (out.enHasMarker) {
            ...
            if (out.hasIPrefix) {
                ...
                out.suggestion := "add_i"
            }
        }
        return out
    }
    out.suggestion := "unknown"
    return out
}

ApplyMarkerSuggestion(itText, outObj) {
    if (!outObj.found)
        return itText
    if (outObj.suggestion = "leave")
        return itText
    if (outObj.suggestion = "add_i") {
        newMarker := "^i" . outObj.base
        newText := RegExReplace(itText, RegExEscape(outObj.raw), newMarker, , 1)
        return newText
    }
    return itText
}
...
for i, row in dataNew {
    ...
    if (!hasEnMarker) {
        ...
        newMarker := currentItMarker ; Lascia com'è se marcatore presente in EN
    }
    if (newMarker != "") {
        if (!hasEnMarker) {
        ...
        if (strippedNoMarker == "") {
            ...
            ; Aggiunto: Rimuovi eventuali marker duplicati (es. ^fd^imd -> ^imd)
            itStrip := RegExReplace(itStrip, "(\^[A-Za-z]{1,4})$", "") ; Rimuove marker finale se presente
...
MsgBox, 64, File Lua Generati, Generati %zoneCount% traduzioni per zone_it.lua e %npcCount% traduzioni per npc_it.luannControlla %debugLog% per il contenuto esatto., 4
...
BuildRow:
    ...
    if (varconfronto <> "") {
        ...
        ; === NUOVA LOGICA MARCATORI ===
        enText := engMap.HasKey(key) ? engMap[key] : ""
        itRaw := textItOrig
        ; Classifica e applica suggerimento
        markerInfo := ClassifyMarker(itRaw, enText)
        itNormalized := ApplyMarkerSuggestion(itRaw, markerInfo)
        ; Log della correzione se modificato
        ...
        if (!match) {
            ...
            srcIt := textItOrig
            ; Normalizza prima eventuali surrogati per avere comportamento consistente
...
{
    ; Costruisci debugMsg SOLO qui (caso grave: fallback su inglese)
    debugMsg .= "NESSUN MATCH e NON VALIDO (non fixabile) - Usa inglesen"
    textIt := textEng
    ...
    logContent .= "MISMATCH VIRGOLETTE GRAVE per chiave " . key . ":`n" . debugMsg . "`n`n"
}
else
if (!match && hasValid)
{
    ; Costruisci debugMsg SOLO qui (mismatch ma valido)
    debugMsg .= "NESSUN MATCH ma VALIDO SECONDO LOGICA - Usa italianon"
    ...
    debugMsg .= "Riga italiana originale: """ . var1 . """,""" . var2 . """,""" . var3 . """,""" . var4 . """,""" . textItOrig . """n"
    logContent .= "MISMATCH VIRGOLETTE (valido ma da verificare) per chiave " . key . ":`n" . debugMsg . "`n`n"
} else
if (anyFixed)
{
    ; Costruisci debugMsg SOLO qui (correzione automatica applicata)
    debugMsg .= "MATCH CONFERMATO ma CORREZIONE APPLICATA - Usa italiano"
    debugMsg .= "Riga italiana originale: """ . var1 . """,""" . var2 . """,""" . var3 . """,""" . var4 . """,""" . textItOrig . """n"
    logContent .= "CORREZIONE AUTOMATICA per chiave " . key . ":`n" . debugMsg . "`n`n"
}
}
else {
    ; NIENTE: match perfetto, non loggare nulla → risparmia memoria
}
