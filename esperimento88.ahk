...
BuildRow:
    ...
    if (varconfronto <> "") {
        ...
        ; === CORREZIONE MARCATORI - VERSIONE MIGLIORATA ===
        currentItMarker := ExtractMarkerFromString(textItOrig)
        hasItMarker := (currentItMarker != "")
       
        if (hasItMarker) {
            ; Normalizza il marcatore esistente
            itNormalized := NormalizeMarkerInString(textItOrig)
            currentItMarker := ExtractMarkerFromString(itNormalized)
           
            ; Ottieni il testo inglese
            enText := engMap.HasKey(key) ? engMap[key] : ""
            enMarker := ExtractMarkerFromString(enText)
           
            ; Valorizza aggiungii: 1 se NO marcatore in EN
            aggiungii := (enMarker == "") ? 1 : 0
           
            ; Determina il marcatore corretto
            chosen := DecideMarker(key, textItOrig, textFr, textSp)
           
            ; Se abbiamo un marcatore da FR/ES, usalo, altrimenti analizza la parola
            if (chosen != "") {
                newMarker := chosen
            } else {
                ; Analizza il genere dalla parola italiana
            ...
            ; Applica la correzione solo se il marcatore è cambiato o se dobbiamo aggiungere la "i", ma solo se aggiungii = 1 (no marker in EN)
            if (aggiungii = 1 && newMarker != "" && (newMarker != currentItMarker || aggiungii = 1)) {
                ; Se aggiungii = 1, aggiungi "i" davanti
                finalMarker := (aggiungii = 1) ? "^i" . newMarker : "^" . newMarker
               
                ; Rimuovi il marcatore esistente e aggiungi il nuovo
                itStrip := RegExReplace(itNormalized, "\s*\^[A-Za-z]{1,4}$", "")
            ...
            ; Se aggiungii = 1, esegui il blocco StrReplace per tutti i marcatori ESO
            if (aggiungii = 1) {
