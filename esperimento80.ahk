...
NormalizeMarkerInString(str) {
    ...
    if (str = "") 
        return ""
    ; Cerca primo marker del tipo ^LETTERS (1-4) seguito da word boundary
    if RegExMatch(str, "\^([A-Za-z]{1,4})\b", match) {
        ; match contiene l'intera occorrenza, es. "^FM" o "^m"
        marker := SubStr(match, 2)        ; rimuove '^'
        StringLower, lowerMarker, marker ; canonicalizza
        ; se ho una mappatura la uso, altrimenti uso lowerMarker
        replMarker := markerMap.HasKey(lowerMarker) ? markerMap[lowerMarker] : lowerMarker
        ; sostituisco SOLO la prima occorrenza trovata
        return RegExReplace(str, "\^([A-Za-z]{1,4})\b", "^" . replMarker)
    }
    return str
}
ExtractMarkerFromString(str) {
    if (str = "")
        return ""
    if RegExMatch(str, "\^([A-Za-z]{1,4})\b", match) {
        marker := SubStr(match, 2)
        StringLower, lowerMarker, marker
        return "^" . lowerMarker
    }
    return ""
}
...
for i, row in dataNew {
    ...
    if (chosen != "") {
        ...
        if (currentItMarker = "") {
            ...
            if (stripped != "") {
                itNormalized := RTrim(itNormalized) . chosen
            }
        } else if (currentItMarker != chosen) {
            itStrip := RegExReplace(itNormalized, "\^[A-Za-z]{1,4}\s*$", "")
            stripped := Trim(itStrip)
            if (stripped != "") {
                itNormalized := RTrim(itStrip) . chosen
            } else {
                itNormalized := ""
            }
        }
...
if (varconfronto <> "") {
    ...
    if (chosen != "") {
        currentMarker := ExtractMarkerFromString(itNormalized)

        ; rimuove un eventuale marker finale del tipo ^xxx (es ^m ^fd ecc.)
        ; regex corretta: caret escapato, lettere (1-4), eventuali spazi finali
        itStrip := RegExReplace(itNormalized, "\^[A-Za-z]{1,4}\s*$", "")

        strippedNoMarker := Trim(itStrip)

        ; se dopo aver tolto il marker non resta testo => la stringa era solo marker (o vuota)
        if (strippedNoMarker = "") {
            markerToUse := chosen
            ; se engMap[key] contiene il nome inglese base, aggiungi il marker scelto
            itNormalized := engMap[key] . markerToUse
            corrRow := """" . var1 . """,""" . var2 . """,""" . var3 . """,""" . var4 . """,""" . itNormalized . """`n"
            corrContent .= corrRow
        } else if (currentMarker != chosen) {
            ; ricompone il nome senza il marker vecchio e aggiunge il marker scelto
            itNormalized := RTrim(itStrip) . chosen
            corrRow := """" . var1 . """,""" . var2 . """,""" . var3 . """,""" . var4 . """,""" . itNormalized . """`n"
            corrContent .= corrRow
        }
...
DecideMarker(key, itRaw, frRaw, spRaw) {
    global markerMap
    ; normalizza le stringhe di confronto
    ...
    ; pulisci (senza caret) per confronto
    ...
    if (frClean != "" && spClean != "" && frClean = spClean)
        return "^" . frClean
    ; conflitto -> non modificare (comportamento conservativo)
    return (frClean != "" ? "^" . frClean : (spClean != "" ? "^" . spClean : ""))
}