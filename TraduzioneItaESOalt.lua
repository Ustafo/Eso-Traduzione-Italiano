...
-- ATTENZIONE: comportamento intenzionale non ovvio
-- Il controllo seguente replica esattamente il comportamento del vecchio addon funzionante.
-- Non sostituire con una comparazione diretta semplice come
--   if location and (location.cosmic == isTamriel) then
-- perché, nel contesto attuale, la normalizzazione dei valori booleani
-- è necessaria per ottenere il risultato desiderato (mostrare i nomi dei continenti).
-- La variante "logica" sembra corretta ma cambia il risultato a causa di valori non booleani/nil
-- in uso da altre parti dell'addon; modifiche superficiali qui possono far sparire i nomi sulla mappa.
-- Se si vuole ristrutturare la logica in modo sicuro, prima:
-- 1) aggiungere test automatici che coprano mappe continentali e mappe di zona;
-- 2) verificare che isTamriel e location.cosmic siano sempre booleani in tutti i percorsi;
-- 3) aggiornare la condizione e rieseguire i test.
-- Lasciare questo commento e contattare lo sviluppatore responsabile prima di qualsiasi cambiamento magari indicando la soluzione corretta :-)
-----------------------------------------------------------------------			
local isTam = (isTamriel == true)
local locCosmic = (location.cosmic == true)
if location and ((not locCosmic) == isTam) then
----------------------------------------------------------------------------------			
